    ...
    <div className="member-history-chart" aria-label="个人历史分数和排名趋势">
      <ResponsiveContainer width="100%" height={290}>
        <LineChart data={trend} margin={{ top: 14, right: 12, bottom: 4, left: -16 }}>
          ...
          <YAxis yAxisId="score" axisLine={false} tickLine={false} tick={{ fill: "#766c5c", fontSize: 11 }} />
          <YAxis yAxisId="rank" orientation="right" reversed domain={[1, "dataMax + 2"]} allowDecimals={false} axisLine={false} tickLine={false} tick={{ fill: "#766c5c", fontSize: 11 }} />
          <Tooltip contentStyle={{ borderRadius: 12, border: "1px solid rgba(80,66,44,.15)", background: "#fffaf0" }} />
          <Line yAxisId="score" type="monotone" dataKey="score" name="分数" stroke="#df662d" strokeWidth={3} dot={{ r: 4, fill: "#df662d" }} />
          <Line yAxisId="rank" type="monotone" dataKey="rank" name="排名" stroke="#3f6c55" strokeWidth={2} strokeDasharray="5 4" dot={{ r: 3, fill: "#3f6c55" }} />
        </LineChart>
      </ResponsiveContainer>
    </div>
  );
