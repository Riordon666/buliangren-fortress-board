  ...
  return (
    <>
      <nav className="mobile-primary-nav" aria-label="手机主导航">
        {primary.map((item) => { const Icon = item.icon; const active = pathname.startsWith(item.href); return <Link key={item.href} href={item.href} prefetch={false} className={active ? "active" : ""}><span><Icon size={20} />{item.href === "/packages" && packageAlert && <i />}</span><small>{item.label}</small></Link>; })}
      ...
      {open && <div className="mobile-more-backdrop" onClick={() => setOpen(false)}>
        <section className="mobile-more-sheet" onClick={(event) => event.stopPropagation()}>
          ...
          <div>{more.map((item) => { const Icon = item.icon; return <Link key={item.href} href={item.href} prefetch={false} className={pathname.startsWith(item.href) ? "active" : ""} onClick={() => setOpen(false)}><Icon size={19} /><span>{item.label}</span></Link>; })}</div>
