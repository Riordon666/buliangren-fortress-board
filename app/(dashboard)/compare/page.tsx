...
import { DeferredMemberComparison } from "@/components/deferred-charts";
...
export default async function ComparePage() {
  ...
  const members = getMembers(false)
    .filter((member) => member.accountType === "member")
  ...
  const initialIds = [
    ...(user.accountType === "member" ? [user.id] : []),
    ...leaders.filter((id) => id !== user.id)
  ].slice(0, 3);
  ...
  return (
    <div className="page-stack compare-page">
      ...
      <DeferredMemberComparison members={members} trends={trends} initialIds={initialIds} />
    </div>
  );
}
